    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Game Results',
          text: `I just finished a game! ${winner ? `${winner.name} won with ${winner.score} points!` : 'Check out the results!'}`,
      ...
      const shareText = `Game Results - ${winner ? `${winner.name} won with ${winner.score} points!` : 'Game completed!'}`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-pink-600 to-red-500 flex items-center justify-center p-4">
      {showConfetti && (
        <div className="fixed inset-0 pointer-events-none z-50">
          ...
          <div className="animate-bounce absolute top-20 right-20 text-4xl" style={{animationDelay: '0.5s'}}>🎊</div>
          <div className="animate-bounce absolute bottom-20 left-20 text-4xl" style={{animationDelay: '1s'}}>✨</div>
          <div className="animate-bounce absolute bottom-10 right-10 text-4xl" style={{animationDelay: '1.5s'}}>🏆</div>
        </div>
      )}
